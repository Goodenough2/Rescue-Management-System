...
import {goodsStorageModes} from '../database'
import {cells, containers, cellTypes} from '../databases/warehouse'
...
export const cellController = [
  ...
  {
    url: '/cell/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = cells.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    ...
    response: config => {
      try {
        const data = cells.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // 冲突检测
        if (cells.some(item => item.name === data.name)) {
          return { ...responses.createConflict, message: '已存在同名的单元格' }
        }

        // id
        data.id = Mock.Random.guid().replace(/-/g, '')
        // prev item
        const prev = cells.sort((a, b) => {
        ...
        // TypeId
        data.cellType = cellTypes.find(item => item.id === data.typeId)
        // containerId
        data.container = containers.find(item => item.id === data.containerId)
        // goodsCategoryId
        data.goodsCategoryClassification = goodsCategoryClassifications.find(item => item.id === data.goodsCategoryId)
  ...
  // 排序上移
  {
    ...
    response: config => {
      try {
        ...
        const prevDatas = cells.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = cells.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        // if (stores.some(item => item.name === data.name && item.id !== data.id)) {
        ...
        if (oldData.typeId !== data.typeId) {
          oldData.cellType = cellTypes.find(item => item.id === data.typeId)
        }
        oldData.typeId = data.typeId
        if (oldData.containerId !== data.containerId) {
          oldData.container = containers.find(item => item.id === data.containerId)
        }
        oldData.containerId = data.containerId
        oldData.rfidCode = data.rfidCode
        oldData.barCode = data.barCode
        oldData.enabled = data.enabled
        oldData.canMix = data.canMix
        if (oldData.goodsCategoryId !== data.goodsCategoryId) {
          oldData.goodsCategoryClassification = goodsCategoryClassifications.find(item => item.id === data.goodsCategoryId)
        }
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = cells.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
