...
import { warehouses } from '../databases/warehouse'
...
export const storeController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        const { code, name, enabled } = config.body.query

        // 查询
        const queryList = warehouses.filter(item => {
  ...
  {
    url: '/warehouse/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = warehouses.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/warehouse/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = warehouses.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // 冲突检测
        // if (stores.some(item => item.name === data.name)) {
        ...
        // prev item
        const prev = warehouses.sort((a, b) => {
        ...
        data.goodsCategoryClassification = goodsCategoryClassifications.find(item => item.id === data.categoryId)
        data.user = users.find(item => item.id === data.managerId)
  ...
  // 排序上移
  {
    ...
    response: config => {
      try {
        ...
        const prevDatas = warehouses.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = warehouses.find(item => item.id === data.id)
        ...
        if (oldData.containerId !== data.containerId) {
          oldData.goodsCategoryClassification = goodsCategoryClassifications.find(item => item.id === data.containerId)
        }
        ...
        if (oldData.managerId !== data.managerId) {
          oldData.user = users.find(item => item.id === data.managerId)
        }
        oldData.managerId=data.managerId
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = warehouses.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
