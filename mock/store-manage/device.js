...
import { devices } from '../databases/warehouse'
...
export const deviceController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        // 查询
        const queryList = devices.filter(item => {
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/device/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = devices.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        //冲突检测
        if (devices.some(item => item.name === data.name)) {
          return { ...responses.createConflict, message: '已存在同名的设备' }
        }
        // id
        data.id = Mock.Random.guid().replace(/-/g, '')
        // prev item
        const prev = devices.sort((a, b) => {
  ...
  // 排序上移
  {
    url: '/device/[\\w\\d]{32}/rankup',
    type: 'patch',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = devices.find(item => item.id === id)
        // 此处应为同一仓库类别下的仓库排序移动，不同类别下的仓库，sort值没有比较依据
        // 因mock数据的问题，此处未实现同一仓库类别下的仓库排序上移功能
        const prevDatas = devices.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = devices.find(item => item.id === data.id )
        if (!oldData) return { ...responses.updateDataNotFound }

       // 冲突检测
        if (devices.some(item => item.name === data.name && item.id !== data.id)) {
          return { ...responses.updateConflict, message: '已存在同名的设备' }
        }
