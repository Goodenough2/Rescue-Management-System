...
import { cellTypes } from '../databases/warehouse'
import { getUrlSection } from '../utils'

export const cellTypeController = [
  // 获取列表数据（查询、翻页）
  {
    url: '/cellType/list',
    type: 'post',
    response: config => {
      try {
        ...
        // 查询
        const queryList = cellTypes.filter(item => {
  ...
  {
    url: '/cellType/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = cellTypes.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/cellType/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = cellTypes.map(({ id, name }) => ({ id, name }))
  ...
  // 排序上移
  {
    ...
    response: config => {
      try {
        ...
        const prevDatas = cellTypes.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = cellTypes.find(item => item.id === data.id)
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = cellTypes.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
