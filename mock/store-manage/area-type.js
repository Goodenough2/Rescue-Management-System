...
import { areaTypes } from '../databases/warehouse'
import { getUrlSection } from '../utils'

export const areaTypeController = [
  // 获取列表数据（查询、翻页）
  {
    url: '/areaType/list',
    type: 'post',
    response: config => {
      try {
        ...
        const { name, description } = config.body.query

        // 查询
        const queryList = areaTypes.filter(item => {
  ...
  // 获取区域类型的下拉数据
  {
    url: '/areaType/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = areaTypes.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // 冲突检测
        if (areaTypes.some(item => item.name === data.name)) {
  ...
  // 排序上移
  {
    ...
    response: config => {
      try {
        ...
        const prevDatas = areaTypes.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = areaTypes.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        if (areaTypes.some(item => item.name === data.name && item.id !== data.id)) {
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = areaTypes.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
