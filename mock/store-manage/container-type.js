...
import { containerTypes } from '../databases/warehouse'
import { getUrlSection } from '../utils'

export const containerTypeController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        // 查询
        const queryList = containerTypes.filter(item => {
  ...
  {
    url: '/containerType/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = containerTypes.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/containerType/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = containerTypes.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    url: '/containerType/create',
    type: 'post',
    response: config => {
      try {
        const data = config.body
        // 冲突检测
        if (containerTypes.some(item => item.name === data.name)) {
          return { ...responses.createConflict, message: '已存在同名的货柜类型' }
        }

        // id
        data.id = Mock.Random.guid().replace(/-/g, '')
        // prev item
        const prev = containerTypes.sort((a, b) => {
  ...
  // 排序上移
  {
    url: '/containerType/[\\w\\d]{32}/rankup',
    type: 'patch',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        ...
        const prevDatas = containerTypes.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    url: '/containerType/update',
    type: 'put',
    response: config => {
      try {
        const data = config.body
        const oldData = containerTypes.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        if (containerTypes.some(item => item.name === data.name && item.id !== data.id)) {
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = containerTypes.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
