...
import { areas, areaTypes, warehouses } from '../databases/warehouse'
...
export const areaController = [
  // 获取区域列表数据
  {
    ...
    response: config => {
      try {
        ...
        const { code, name, areaTypeId, houseId, enabled } = config.body.query

        // 查询
        const queryList = areas.filter(item => {
          if (code && !item.code.includes(code)) return false
          if (name && !item.name.includes(name)) return false
          if (areaTypeId && item.areaType.id !== areaTypeId) return false
          if (houseId && item.warehouse.id !== houseId) return false
  ...
  // 获取下拉列表选项形式的数据(区域名称),此时传过来一个storeId,获取该仓库下的区域
  {
    url: '/area/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = areas.map(({ id, name }) => ({ id, name }))
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        ...
        if (oldData.houseId !== data.houseId) {
          oldData.warehouse = warehouses.find(item => item.id === data.houseId)
        }
        oldData.areaTypeId = data.areaTypeId
        oldData.houseId = data.houseId
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // houseId
        data.warehouse = warehouses.find(item => item.id === data.houseId)
