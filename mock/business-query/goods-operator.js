...
import { goodsOperators } from '../database'
import {getUrlSection} from "../utils";

export const goodsOperatorController = [
  // 获取区域列表数据
  {
    url: '/goods-operator/list',
    type: 'post',
    response: config => {
      try {
        const { current = 1, size = 20 } = config.body.page
        const { property, direction } = config.body.sort
        const { startTime, endTime, goodsType, type, operatorCode, rfid , goodsName} = config.body.query

        console.log(config.body.query)
        // 查询
        const queryList = goodsOperators.filter(item => {
          if (goodsType && item.goods.type.id !== goodsType) return false
          if (type && item.type !== type * 1) return false
          if (operatorCode && item.operatorCode !== operatorCode) return false
          if (rfid && item.rfid.rfid !== rfid * 1) return false
          if (goodsName && item.goods.name !== goodsName) return false
          return true
        })
  ...
  {
    url: '/goods-operator/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = goodsOperators.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
