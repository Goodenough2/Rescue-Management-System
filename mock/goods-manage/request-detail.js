...
export const requestDetailController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        const { requestCode, goodsCategoryId, operatorId, status  } = config.body.query
        // 查询
        const queryList = requestDetails.filter(item => {
          if (requestCode && item.request.code !== requestCode ) return false
  ...
  {
    url: '/request-detail/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = requestDetails.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/request-detail/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = requestDetails.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    url: '/request-detail/create',
    type: 'post',
    response: config => {
      try {
        const data = config.body.requestDetail
        // // 冲突检测
        // if (requestsDea.some(item => item.code === data.code)) {
        //   return { ...responses.createConflict, message: '已存在同名的领料单编码' }
        // }

        // id
        data.id = Mock.Random.guid().replace(/-/g, '')
        data.request = requests.find(item => item.id === data.requestId)
  ...
  // 排序上移
  {
    url: '/reques-detailt/[\\w\\d]{32}/rankup',
    type: 'patch',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
  ...
  // 更新数据
  {
    url: '/request-detail/update',
    type: 'put',
    response: config => {
      try {
        const data = config.body
        const oldData = requestDetails.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        if (requestDetails.some(item => item.name === data.name && item.id !== data.id)) {
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = requestDetails.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
