...
import { goodsUnits } from '../databases/goods-type'
import { getUrlSection } from '../utils'

export const goodsUnitController = [
  // 获取列表数据（查询、翻页）
  {
    url: '/goodsUnit/list',
    type: 'post',
    response: config => {
      try {
        ...
        // 查询
        const queryList = goodsUnits.filter(item => {
        ...
        return {
          ...
          data: {
            ...
            //     isBasic,
            //     storageModeld,
            //     storageModel:{ name: storageModel.name },
            //     convertDesc
            //   }))
          }
        }
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/goodsUnit/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = goodsUnits.map(({ id, name }) => ({ id, name }))
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = goodsUnits.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
