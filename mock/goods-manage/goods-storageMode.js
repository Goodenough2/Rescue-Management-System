...
import {goodsCategories, goodsStorageModes, goodsUnits} from "../databases/goods-type";
import { getUrlSection } from '../utils'
import {areas, areaTypes, containerTypes} from "../databases/warehouse";

export const goodsStorageModelController = [
  // 获取列表数据（查询、翻页）
  {
    url: '/goods-storageMode/list',
    type: 'post',
    response: config => {
      try {
        const { current = 1, size = 20 } = config.body.page
        const { property, direction } = config.body.sort
        const { name, goodsCategoryId, unitId  } = config.body.query
        // 查询
        const queryList = goodsStorageModes.filter(item => {
          if (name && !item.name.includes(name)) return false
          if (goodsCategoryId && item.goodsCategory.id !== goodsCategoryId) return false
          if (unitId && item.goodsUnit.id !== unitId) return false
          return true
        })
  ...
  {
    url: '/goods-storageMode/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = goodsStorageModes.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/goods-storageMode/selectlist',
    type: 'get',
    response: config => {
      try {
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // 冲突检测
        if (goodsStorageModes.some(item => item.name === data.name)) {
          return { ...responses.createConflict, message: '已存在同名的存储方式' }
        }

        // id
        data.id = Mock.Random.guid().replace(/-/g, '')
        // prev item
        const prev = goodsStorageModes.sort((a, b) => {
  ...
  // 排序上移
  {
    url: '/goods-storageMode/[\\w\\d]{32}/rankup',
    type: 'patch',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        ...
        const prevDatas = goodsStorageModes.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    url: '/goods-storageMode/update',
    type: 'put',
    response: config => {
      try {
        const data = config.body
        const oldData = goodsStorageModes.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        if (goodsStorageModes.some(item => item.name === data.name && item.id !== data.id)) {
          return { ...responses.updateConflict, message: '已存在同名的存储方式' }
        }

        // 需要更新的字段，不采取整个对象更新的方式
        oldData.name = data.name
        oldData.groupNum = data.groupNum
        ...
        oldData.volume = data.length * data.width * data.height
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = goodsStorageModes.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
          ...
          goodsStorageModes.splice(index, 1)
        }
