...
import { suppliers } from '../databases/both'
import { goodsCategoryClassifications } from '../databases/goods-type'
import { goodsColours, goodsUnits } from '../databases/goods-type'
import { getUrlSection } from '../utils'

export const goodsCategoryController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        const { code, name, batch, colourId, supplierId, classificationId, unitId } = config.body.query

        // 查询
        const queryList = goodsCategories.filter(item => {
          if (code && !item.code.includes(code)) return false
          if (name && !item.name.includes(name)) return false
          if (batch && !item.batch.includes(batch)) return false
          if (classificationId && item.classificationId !== classificationId) return false
          if (unitId && item.unitId !== unitId) return false
          if (colourId && item.colourId !== colourId) return false
  ...
  {
    url: '/goodsCategory/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = goodsCategories.find(item => item.id === id)
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/goodsCategory/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = goodsCategories.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // 冲突检测
        if (goodsCategories.some(item => item.name === data.name)) {
          return { ...responses.createConflict, message: '已存在同名的货物类别' }
        }
        if (goodsCategories.some(item => item.code === data.code)) {
          return { ...responses.createConflict, message: '已存在相同的货物类别编号' }
        }

        // id
        data.id = Mock.Random.guid().replace(/-/g, '')
        // prev item
        const prev = goodsCategories.sort((a, b) => {
          return b.sort - a.sort
        }).slice(0, 1)[0]
        // sort次序
        data.sort = prev.sort + 1
        data.sort = prev.sort + 1
        data.goodsUnit = goodsUnits.find(item => item.id === data.unitId)
        data.goodsCategoryClassification = goodsCategoryClassifications.find(item => item.id === data.classificationId)
        data.supplier = suppliers.find(item => item.id === data.supplierId)
        data.goodsColour = goodsColours.find(item => item.id === data.colourId)
  ...
  // 排序上移
  {
    url: '/goodsCategory/[\\w\\d]{32}/rankup',
    type: 'patch',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        ...
        const prevDatas = goodsCategories.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = goodsCategories.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        if (goodsCategories.some(item => item.name === data.name && item.id !== data.id)) {
          return { ...responses.updateConflict, message: '已存在同名的货物类型' }
        }
        if (goodsCategories.some(item => item.code === data.code && item.id !== data.id)) {
          return { ...responses.updateConflict, message: '已存在相同的货物编号' }
        }
        ...
        oldData.unitPrice = data.unitPrice
        oldData.unitWeight = data.unitWeight

        oldData.description = data.description

        if (oldData.unitId !== data.unitId) {
          oldData.goodsUnit = goodsUnits.find(item => item.id === data.unitId)
        }
        if (oldData.classificationId !== data.classificationId) {
          oldData.goodsCategoryClassification = goodsCategoryClassifications.find(item => item.id === data.classificationId)
        }
        if (oldData.supplierId !== data.supplierId) {
          oldData.supplier = suppliers.find(item => item.id === data.supplierId)
        }
        if (oldData.colourId !== data.colourId) {
          oldData.goodsColour = goodsColours.find(item => item.id === data.colourId)
        }
        oldData.unitId = data.unitId
        oldData.colourId = data.colourId
        oldData.supplierId = data.supplierId
        oldData.classificationId = data.classificationId
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = goodsCategories.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
          ...
          goodsCategories.splice(index, 1)
        }
