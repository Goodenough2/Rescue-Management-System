...
export const goodsCategoryClassificationController = [
  // 获取列表数据（查询、翻页）
  {
    url: '/goods-category-classification/list',
    type: 'post',
    response: config => {
      try {
        const { current = 1, size = 20 } = config.body.page
        const { property, direction } = config.body.sort
        const { code, name  } = config.body.query
        // 查询
        const queryList = goodsCategoryClassifications.filter(item => {
  ...
  {
    url: '/goods-category-classification/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = goodsCategoryClassifications.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/goods-category-classification/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = goodsCategoryClassifications.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    url: '/goods-category-classification/create',
    type: 'post',
    response: config => {
      try {
        ...
        // 冲突检测
        if (goodsCategoryClassifications.some(item => item.name === data.name)) {
        ...
        // prev item
        const prev = goodsCategoryClassifications.sort((a, b) => {
  ...
  // 排序上移
  {
    url: '/goods-category-classification/[\\w\\d]{32}/rankup',
    type: 'patch',
    response: config => {
      console.log("11111")
      try {
        ...
        const prevDatas = goodsCategoryClassifications.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    url: '/goods-category-classification/update',
    type: 'put',
    response: config => {
      try {
        const data = config.body
        const oldData = goodsCategoryClassifications.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        if (goodsCategoryClassifications.some(item => item.name === data.name && item.id !== data.id)) {
          return { ...responses.updateConflict, message: '已存在同名的货物类别分类' }
        }
        if (goodsCategoryClassifications.some(item => item.code === data.code && item.id !== data.id)) {
          return { ...responses.updateConflict, message: '已存在相同的编号' }
        }
        ...
        oldData.parentId = data.parentId
  ...
  // 删除、批量删除数据
  {
    url: '/goods-category-classification/delete',
    type: 'delete',
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = goodsCategoryClassifications.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
          ...
          goodsCategoryClassifications.splice(index, 1)
        }
