...
export const orderRequistController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        const {  code, reasonType, orderCode, createrId, status  } = config.body.query
        // 查询
        const queryList = requests.filter(item => {
          if (code && item.code !== code ) return false
          if (orderCode && !item.order.code.includes(orderCode)) return false
          if (createrId && item !== createrId) return false
          if (status && item.status !== status * 1) return false
          if (reasonType && item.reasonType !== reasonType * 1) return false
          return true
        })
  ...
  {
    url: '/request/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = requests.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/request/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = requests.map(({ id, code }) => ({ id, code }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        const data = config.body.request
        // 冲突检测
        if (requests.some(item => item.code === data.code)) {
        ...
        data.user = users.find(item => item.id === data.createrId)
        requests.push(data)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        ...
        const oldData = requests.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        if (requests.some(item => item.code === data.code && item.id !== data.id)) {
          return { ...responses.updateConflict, message: '已存在相同的领料单编号' }
        }
