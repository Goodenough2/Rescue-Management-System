...
import { goodsColours } from '../databases/goods-type'
export const goodsColorController = [
  // 获取列表数据（查询、翻页）
  {
    url: '/goodsColor/list',
    type: 'post',
    response: config => {
      try {
        ...
        // 查询
        const queryList = goodsColours.filter(item => {
  ...
  {
    url: '/goodsColor/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = goodsColours.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/goodsColor/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = goodsColours.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // 冲突检测
        if (goodsColours.some(item => item.name === data.name)) {
        ...
        // prev item
        const prev = goodsColours.sort((a, b) => {
  ...
  // 排序上移
  {
    ...
    response: config => {
      try {
        ...
        const prevDatas = goodsColours.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    url: '/goodsColor/update',
    type: 'put',
    response: config => {
      try {
        const data = config.body
        const oldData = goodsColours.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        if (goodsColours.some(item => item.name === data.name && item.id !== data.id)) {
          return { ...responses.updateConflict, message: '已存在同名的颜色' }
        }
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = goodsColours.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
