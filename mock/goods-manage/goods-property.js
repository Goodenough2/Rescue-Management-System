...
import { goodsProperties } from "../databases/goods-type";
export const goodsPropertyController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        const { valueType, name  } = config.body.query
        // 查询
        const queryList = goodsProperties.filter(item => {
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/goods-property/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = goodsProperties.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // 冲突检测
        if (goodsProperties.some(item => item.name === data.name)) {
          return { ...responses.createConflict, message: '已存在货物扩展属性名称' }
        }
  ...
  // 更新数据
  {
    url: '/goods-property/update',
    type: 'put',
    response: config => {
      try {
        const data = config.body
        const oldData = goodsProperties.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        if (goodsProperties.some(item => item.name === data.name && item.id !== data.id)) {
          return { ...responses.updateConflict, message: '已存在同名的货物扩展属性名称' }
        }
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = goodsProperties.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
          ...
          goodsProperties.splice(index, 1)
        }
