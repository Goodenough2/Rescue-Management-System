...
import { goodsChecks } from '../database'
import { getUrlSection } from '../utils'

export const goodsCheckController = [
  // 获取列表数据（查询、翻页）
  {
    url: '/goods-check/list',
    type: 'post',
    response: config => {
      try {
        const { current = 1, size = 20 } = config.body.page
        const { property, direction } = config.body.sort
        const { code, name, status } = config.body.query

        // 查询
        const queryList = goodsChecks.filter(item => {
          if (code && !item.purchase.purchaseOrder.code.includes(code)) return false
          if (name && !item.purchase.purchaseOrder.supplier.name.includes(name)) return false
  ...
  {
    url: '/goods-check/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = goodsChecks.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    ...
    response: config => {
      try {
        const data = stores.map(({ id, name }) => ({ id, name }))
  ...
  // 排序上移
  {
    url: '/store/[\\w\\d]{32}/rankup',
    type: 'patch',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = stores.find(item => item.id === id)
        // 此处应为同一仓库类别下的仓库排序移动，不同类别下的仓库，sort值没有比较依据
        // 因mock数据的问题，此处未实现同一仓库类别下的仓库排序上移功能
        const prevDatas = stores.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = stores.find(item => item.id === data.id)
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = stores.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
