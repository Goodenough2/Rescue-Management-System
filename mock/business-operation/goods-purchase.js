...
import { goodsPurchases } from '../database'
import { getUrlSection } from '../utils'

export const goodsPurchaseController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        const { code } = config.body.query

        // 查询
        const queryList = goodsPurchases.filter(item => {
          if (code && !item.purchaseOrder.code.includes(code)) return false
          return true
        })
  ...
  {
    url: '/goods-purchase/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = goodsPurchases.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 排序上移
  {
    url: '/goods-purchase/[\\w\\d]{32}/rankup',
    type: 'patch',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        ...
        const prevDatas = goodsPurchases.filter(item => item.sort < data.sort)
