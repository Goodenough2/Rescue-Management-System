...
export const userGroupController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        const { approval } = config.body.query
        console.log("approval",approval)
        // 查询
        const queryList = groups.filter(item => {
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/user-group/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = groups.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        //冲突检测
        if (groups.some(item => item.name === data.name)) {
  ...
  // 排序上移
  {
    ...
    response: config => {
      try {
        ...
        const prevDatas = groups.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = groups.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        if (groups.some(item => item.name === data.name && item.id !== data.id)) {
          return { ...responses.updateConflict, message: '已存在同名的用户组' }
        }

        // 需要更新的字段，不采取整个对象更新的方式
        oldData.name = data.name
        oldData.description = data.description
        oldData.approval = data.approval
