...
export const userController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        const { uname, nick } = config.body.query
        // 查询
        const queryList = users.filter(item => {
          if (uname && !item.uname.includes(uname)) return false
          if (nick && !item.nick.includes(nick)) return false
          return true
        })
  ...
  // 获取下拉列表选项形式的数据
  {
    ...
    response: config => {
      try {
        const data = users.map(({ id, uname }) => ({ id, uname }))
  ...
  // 排序上移
  {
    ...
    response: config => {
      try {
        ...
        const prevDatas = users.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = users.find(item => item.id === data.id)
        ...
        oldData.uname = data.uname
        oldData.nick = data.nick
        oldData.pwd = data.pwd
        oldData.lock = data.lock
