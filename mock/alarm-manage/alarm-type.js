...
import { alarmTypes, alarmGroups } from '../databases/alarm'
import { users } from '../databases/system'
export const alarmTypeController = [
  // 获取列表数据（查询、翻页）
  {
    url: '/alarmType/list',
    type: 'post',
    response: config => {
      try {
        ...
        const { name, alarmByUserId, alarmByGroupId } = config.body.query
        // 查询
        const queryList = alarmTypes.filter(item => {
          if (name && !item.name.includes(name)) return false
          if (alarmByUserId && item.alarmByUserId !== alarmByUserId) return false
          if (alarmByGroupId && item.alarmByGroupId !== alarmByGroupId) return false
          return true
        })
  ...
  {
    url: '/alarmType/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = alarmTypes.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // 冲突检测
        if (alarmTypes.some(item => item.name === data.name)) {
        ...
        // prev item
        const prev = alarmTypes.sort((a, b) => {
        ...
        data.user = users.find(item => item.id === data.alarmByUserId)
        data.alarmGroup = alarmGroups.find(item => item.id === data.alarmByGroupId)
  ...
  // 更新数据
  {
    url: '/alarmType/update',
    type: 'put',
    response: config => {
      try {
        const data = config.body
        const oldData = alarmTypes.find(item => item.id === data.id)
        ...
        if (oldData.alarmByUserId !== data.alarmByUserId) {
          oldData.user = users.find(item => item.id === data.alarmByUserId)
        }
        oldData.alarmByUserId = data.alarmByUserId
        if (oldData.alarmByGroupId !== data.alarmByGroupId) {
          oldData.alarmGroup = alarmGroups.find(item => item.id === data.alarmByGroupId)
        }
        oldData.alarmByGroupId = data.alarmByGroupId
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = alarmTypes.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
