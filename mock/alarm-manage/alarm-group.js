...
import { alarmGroups } from '../databases/alarm'
export const alarmGroupController = [
  // 获取列表数据（查询、翻页）
  {
    url: '/alarmGroup/list',
    type: 'post',
    response: config => {
      try {
        ...
        // 查询
        const queryList = alarmGroups.filter(item => {
  ...
  {
    url: '/alarmGroup/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = alarmGroups.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/alarmGroup/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = alarmGroups.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // 冲突检测
        if (alarmGroups.some(item => item.name === data.name)) {
        ...
        // prev item
        const prev = alarmGroups.sort((a, b) => {
  ...
  // 排序上移
  {
    ...
    response: config => {
      try {
        ...
        const prevDatas = alarmGroups.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = alarmGroups.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        if (alarmGroups.some(item => item.name === data.name && item.id !== data.id)) {
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = alarmGroups.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
