...
import { alarmTypes, alarmInfos } from '../databases/alarm'
import { users } from '../databases/system'
export const alarmInfoController = [
  // 获取列表数据（查询、翻页）
  {
    url: '/alarmInfo/list',
    type: 'post',
    response: config => {
      try {
        ...
        const { alarmTypeId, alarmToUserId, alarmFromUserId, readFlag, dealFlag } = config.body.query
        // 查询
        const queryList = alarmInfos.filter(item => {
          if (alarmTypeId && item.alarmTypeId !== alarmTypeId) return false
          if (alarmToUserId && item.alarmToUserId !== alarmToUserId) return false
          if (alarmFromUserId && item.alarmFromUserId !== alarmFromUserId) return false
          if (readFlag && item.readFlag !== readFlag * 1) return false // bug，数字会变为字符串
          if (dealFlag && item.dealFlag !== dealFlag * 1) return false
          return true
        })
  ...
  {
    url: '/alarmInfo/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = alarmInfos.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/alarmInfo/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = alarmInfos.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // 冲突检测
        // if (alarmInfos.some(item => item.alarmTitle === data.alarmTitle)) {
        //   return { ...responses.createConflict, message: '已存在同名的颜色' }
        // }

        // id
        data.id = Mock.Random.guid().replace(/-/g, '')
        // prev item
        const prev = alarmInfos.sort((a, b) => {
        ...
        data.alarmType = alarmTypes.find(item => item.id === data.alarmTypeId)
        data.alarmToUser = users.find(item => item.id === data.alarmToUserId)
        data.alarmFromUser = users.find(item => item.id === data.alarmFromUserId)
  ...
  // 排序上移
  {
    ...
    response: config => {
      try {
        ...
        const prevDatas = alarmTypes.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = alarmInfos.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // // 冲突检测
        // if (alarmTypes.some(item => item.name === data.name && item.id !== data.id)) {
        //   return { ...responses.updateConflict, message: '已存在同名的警报' }
        // }

        // 需要更新的字段，不采取整个对象更新的方式
        oldData.alarmTitle = data.alarmTitle
        oldData.alarmContent = data.alarmContent
        if (oldData.alarmTypeId !== data.alarmTypeId) {
          oldData.alarmType = alarmTypes.find(item => item.id === data.alarmTypeId)
        }
        oldData.alarmTypeId = data.alarmTypeId
        if (oldData.alarmToUserId !== data.alarmToUserId) {
          oldData.alarmToUser = users.find(item => item.id === data.alarmToUserId)
        }
        oldData.alarmToUserId = data.alarmToUserId
        if (oldData.alarmFromUserId !== data.alarmFromUserId) {
          oldData.alarmFromUser = users.find(item => item.id === data.alarmFromUserId)
        }
        oldData.alarmFromUserId = data.alarmFromUserId
        oldData.readFlag = data.readFlag
        oldData.dealFlag = data.dealFlag
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = alarmInfos.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
