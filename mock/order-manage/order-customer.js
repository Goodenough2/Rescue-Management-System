...
import {customers, orderCustomers, orderTypes, users } from '../database'
import { getUrlSection } from '../utils'

export const orderCustomerController = [
  // console.log('database11111111111111111111111111111111'),
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        const { code/*, typeId, quality, amount, qualityForReal, amountForReal, status, customerId, deliveryTime, operatorId*/ } = config.body.query

        // 查询
        const queryList = orderCustomers.filter(item => {
  ...
  {
    url: '/order-customer/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/order-customer/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = orderCustomers.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // prev item
        const prev = orderCustomers.sort((a, b) => {
  ...
  // 排序上移
  {
    url: '/order-customer/[\\w\\d]{32}/rankup',
    type: 'patch',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = orderCustomers.find(item => item.id === data.id)
        ...
        oldData.deliveryTime = data.deliveryTime

        if (oldData.typeId !== data.typeId) {
          oldData.type = orderTypes.find(item => item.id === data.typeId)
        }
        // if (oldData.statusId !== data.statusId) {
        //   oldData.status = status.find(item => item.id === data.statusId)
        // }
        if (oldData.customerId !== data.customerId) {
          oldData.customer = customers.find(item => item.id === data.customerId)
        }
        ...
        oldData.customerId = data.customerId
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = orderCustomers.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
