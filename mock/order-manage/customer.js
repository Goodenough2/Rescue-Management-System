...
export const customerController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        // 查询
        const queryList = customers.filter(item => {
  ...
  // 获取区域类型的下拉数据
  {
    ...
    response: config => {
      try {
        const data = customers.map(({ id, name }) => ({ id, name }))
  ...
  // 排序上移
  {
    ...
    response: config => {
      try {
        ...
        const prevDatas = customers.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = customers.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        // if (customers.some(item => item.name === data.name && item.id !== data.id)) {
