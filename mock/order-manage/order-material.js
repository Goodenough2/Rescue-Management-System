...
import { orderMaterials } from '../databases/order'
...
export const orderMaterialController = [
  ...
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        const { detailId, goodsCategoryId, status, operatorId } = config.body.query

        // 查询
        const queryList = orderMaterials.filter(item => {
          if (detailId && item.orderDetail.id !== detailId) return false
  ...
  {
    url: '/order-material/[\\w\\d]{32}/detail',
    type: 'get',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = orderMaterials.find(item => item.id === id)
        if (data) {
          return {
            ...responses.success,
            // 在涉及不应给用户看到的重要数据时，也应抽取相应的属性及数据，不应返回全部数据
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/order-material/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = orderMaterials.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        // order
        data.orderDetail = orderDetails.find(item => item.id === data.detailId)
        // goodsCategory
  ...
  // 排序上移
  {
    ...
    response: config => {
      try {
        ...
        const prevDatas = orderMaterials.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = orderMaterials.find(item => item.id === data.id)
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = orderMaterials.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
