...
export const supplierController = [
  // 获取列表数据（查询、翻页）
  {
    ...
    response: config => {
      try {
        ...
        // 查询
        const queryList = suppliers.filter(item => {
  ...
  // 获取下拉列表选项形式的数据
  {
    url: '/supplier/selectlist',
    type: 'get',
    response: config => {
      try {
        const data = suppliers.map(({ id, name }) => ({ id, name }))
  ...
  // 添加数据
  {
    ...
    response: config => {
      try {
        ...
        //
        // // 冲突检测
        // if (customers.some(item => item.name === data.name)) {
        //   return { ...responses.createConflict, message: '已存在同名的客户' }
        // }

        // id
        data.id = Mock.Random.guid().replace(/-/g, '')
        // prev item
        const prev = suppliers.sort((a, b) => {
  ...
  // 排序上移
  {
    url: '/supplier/[\\w\\d]{32}/rankup/',
    type: 'patch',
    response: config => {
      try {
        const id = getUrlSection(config.url, 2)
        const data = suppliers.find(item => item.id === id)
        // 此处应为同一仓库类别下的仓库排序移动，不同类别下的仓库，sort值没有比较依据
        // 因mock数据的问题，此处未实现同一仓库类别下的仓库排序上移功能
        const prevDatas = suppliers.filter(item => item.sort < data.sort)
  ...
  // 更新数据
  {
    ...
    response: config => {
      try {
        const data = config.body
        const oldData = suppliers.find(item => item.id === data.id)
        if (!oldData) return { ...responses.updateDataNotFound }

        // 冲突检测
        // if (suppliers.some(item => item.name === data.name && item.id !== data.id)) {
        //   return { ...responses.updateConflict, message: '已存在同名的区域类型' }
        // }
        ...
        oldData.address = data.address,
          ...
          oldData.contactDuty = data.contactDuty
  ...
  // 删除、批量删除数据
  {
    ...
    response: config => {
      try {
        ...
        let message = ''
        for (const id of ids) {
          const index = suppliers.findIndex((item) => {
            return item.id === id
          })
          if (index < 0) {
            notFoundCount++
